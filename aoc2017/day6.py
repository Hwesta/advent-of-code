...
def solve(data, flag=False):
    ...
    if flag:
        first_seen = seen.index(tuple(blocks))
        return len(seen) - first_seen
    else:
        return len(seen)
...
if __name__ == '__main__':
    ...
    with open(os.path.join(this_dir, 'day6.input')) as f:
        data = f.read().strip()
    print('It takes', solve(data, False), 'steps to find an infinite loop.')
    print('The loop is', solve(data, True), 'steps long.')
